...
% Preallocate the validation estimates and posterior PSM inputs
nSite = numel(models);
nScaling = numel(Rscaling);
nLocs = numel(locRadii);

Ypost = NaN(nSite, nScaling, nLocs);
tosPost = NaN(nSite, nScaling, nLocs);
sosPost = NaN(nSite, nScaling, nLocs);
...
for k = 1:nLocs
    ...
    for r = 1:nScaling
        ...
        for s = 1:nSite
            ...
            % Record the posterior PSM inputs
            A = output.Amean;
            tosPost(s,r,k) = A(1);
            sosPost(s,r,k) = A(2);
            ...
            Ypost(s,r,k) = models{s}.estimate(A);
...
nccreate(file, 'Y', 'Dimensions', {'sites',nSite}, 'Datatype', 'double');
nccreate(file, 'Ypost',  'Dimensions', {'sites',nSite,'scalings',nScaling,'radii',nLocs}, 'Datatype', 'double');
nccreate(file, 'tos_posterior', 'Dimensions', {'sites',nSite,'scalings',nScaling,'radii',nLocs}, 'Datatype', 'double');
nccreate(file, 'sos_posterior', 'Dimensions', {'sites',nSite,'scalings',nScaling,'radii',nLocs}, 'Datatype', 'double');
...
ncwriteatt(file, 'Y', 'Description', 'The assimilated proxy observations');
...
ncwriteatt(file, 'tos_posterior', 'Description', 'The posterior seasonal sea surface temperatures used to run the knockout PSMs');
ncwriteatt(file, 'tos_posterior', 'Units', 'Celsius');
ncwriteatt(file, 'sos_posterior', 'Description', 'The posterior seasonal sea surface salinities used to run the knockout PSMs');
ncwriteatt(file, 'sos_posterior', 'Units', 'g/kg');
ncwriteatt(file, 'sos_posterior', 'Units_equivalent', 'psu');
...
ncwrite(file, 'tos_posterior', tosPost);
ncwrite(file, 'sos_posterior', sosPost);
