function[] = runNetworkExperiments(tag, piRuns, plioRuns)
...
for n = 1:numel(networkTags)
    for s = 1:numel(ukSeasonTags)
        ...
        assimilate(labels(1), estimates(1), Rtag, piRuns, networks(:,n));
        assimilate(labels(2), estimates(2), Rtag, plioRuns, networks(:,n));
        assimilate(labels(3), estimates(3), Rtag, plioRuns, networks(:,n));
