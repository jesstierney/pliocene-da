...
mod = NaN(length(files),1);
plio = NaN(length(files),1);
age = cell(length(files),1);
mg = cell(length(files),1);
clean = cell(length(files),1);
...
for i = 1:length(files)
    ...
    mod(i) = T.ProxyValue(1);
    plio(i) = mean(T.ProxyValue(T.Age > idx1 & T.Age < idx2),'omitnan');
    age{i} = T.Age;
    mg{i} = T.ProxyValue;
    clean{i} = T.CleaningMethod;
    ydate(i) = min(T.Age(2:end));
    odate(i) = max(T.Age(2:end));
    nDats(i) = length(T.Age(T.Age > idx1 & T.Age < idx2));
    %res(i) = mean(diff(T.Age(T.Age > idx1 & T.Age < idx2)));
end
...
m_coast('patch',[.5 .5 .5]);
m_scatter(lonM,latM,sz,mod,'filled');
...
m_proj('robinson'); hold on;
m_coast('patch',[.5 .5 .5]);
m_scatter(lonS,latS,sz,(log(plio+.24) - log(mod))./.06,'filled');
m_grid;
cptcmap('cbacRdYlBu10','flip',true);
caxis([-5 5]);
colorbar;
...
f3=figure(3); clf;
set(f3,'pos',[100 500 2000 200]);
s1 = 15;
s2 = s1+6;
for i = s1:s2
    s3 = i-s1+1;
    subplot(1,7,s3)
    plot(age{i},mg{i},'color','k','marker','o','markersize',4);
    set(gca,'xlim',[0 5.6]);
    xlabel('age Ma');
    ylabel('mg');
    title(files(i).name);
